 ...
 *
 * Lives in @metoo/shared because three surfaces quote it: the product page
 * estimates it in the browser before anything is in a basket, the cart shows
 * it, and checkout charges it on the server. Two implementations would
 * eventually disagree, and that disagreement is a shop being shown one
 * delivery price and invoiced another.
 *
...
export function shippingFeeMinor(params: {
  ...
  /**
   * True until this retailer has placed an order. Their first delivery is on
   * metoo — the standing welcome offer, applied automatically rather than as
   * a code, because a promotion nobody remembers to type is not a promotion.
   */
  isFirstOrder?: boolean
}): number {
  const { weightGrams, subtotalMinor, isFirstOrder = false } = params
